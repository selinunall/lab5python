...
# print('Initial password: '+reverse(password))
...
def rep_with_upper(string):
    for i in range(len(string)):
        randomLetter=random.choice(string)

    newrandomLetter=randomLetter.upper()
    string=string.replace(randomLetter,newrandomLetter)

    return (string)

def swap_letters(string):
    firstTwo=string[:2]
    lastTwo=string[len(string)-2:]
    middle=string[2:len(string)-2]
    string=lastTwo+middle+firstTwo
    return (string)
